...
themehouse.topics.initWidgets = function () {
    ...
    $threadWidgets.each(function() {
        ...
        if (!$filters.length && (!themehouse.topics.titles || !themehouse.topics.titles[topicType])) {
            return;
        }
        ...
        $threadTopics.each(function() {
            ...
            var topicType = $threadTopic.data('topicType');
            if ($filters.length) {
                var $filter = $filters.filter('[data-topic-id=' + topicId + '][data-topic-type=' + topicType + ']');
                if ($filter.length) {
                ...
                var $topic = themehouse.topics.titles[topicType][topicId];
                if ($topic) {
                    $threadTopic.find('.thTopic--title').text($topic.title);
                    $threadTopic.css('display', '');
                } else {
                    $threadTopic.remove();
                }
